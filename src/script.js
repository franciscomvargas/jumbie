let assetsToLoad = 3; // Number of assets to load (models, textures, etc.)
let assetsLoaded = 0; // Counter for assets that have been loaded
...
function onAssetLoaded() {
    ...
    if(assetsLoaded === assetsToLoad) {
        ...
        // All assets are loaded, hide the loader
        loaderWrapper.classList.add('loaded');
        // Optional: Remove the loader from DOM after transition ends
        loaderWrapper.addEventListener('transitionend', () => {
            loaderWrapper.remove();
            // Make the brand container visible after the loader is hidden
            document.getElementById('brand-container').style.display = 'block';
            
            // Add a slight delay to create a smoother transition effect
            setTimeout(() => {
              document.getElementById('brand-container').style.opacity = 1;
...
function loadModels() {
  ...
  onAssetLoaded();
  console.log("🚀 ~ loadModels ~ assetsLoaded:", assetsLoaded)
}//end loadModels
...
function moveCharacter(){
    characterGroup.position.z < -75 ? (characterGroup.position.z = 75) : (characterGroup.position.z -= .30);
}
  ...
  if(VR){
   ...
   //smoke.rotation.y += .025;
      smoke.children.map((d,i)=>{
        d.position.y =  d.position.y > 5 ? 0 : d.position.y += (i*.0025); // Lower the multiplier for smoother/
      });
...
//set camera position
camera.position.y = 2;  // Previously '1.5'
camera.position.z = -60; // Previously '-25'
camera.position.x = 50; // Previously '50'